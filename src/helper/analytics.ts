...
import { BlockProviderProps, useBlockSelector } from "../components/BlockProvider";
import { TEleganteStore } from "../components/Provider";
...
export const useRecordMenuItemView = () => {
  const { page } = useSelector<TEleganteStore, TEleganteStore>((state) => ({
    page: state?.page,
  }));
...
export const useRecordLinkClick = () => {
  ...
  try {
    const { block_uuid: store_block_uuid } = useBlockSelector<BlockProviderProps, BlockProviderProps>((state) => state);
