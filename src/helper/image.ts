...
export const createImageSrcset = (value: MediaValue) => {
  ...
  const image3x = `${dolosUrl}/${value?.src}${isAmpersand ? "&tr" : "?tr"}=w-1100${calculateCropCoordinates(
    1100,
    value?.coordinates?.desktop
  )}`;
  const image2x = `${dolosUrl}/${value?.src}${isAmpersand ? "&tr" : "?tr"}=w-750${calculateCropCoordinates(
    750,
    value?.coordinates?.tablet
  )}`;
  const image1x = `${dolosUrl}/${value?.src}${isAmpersand ? "&tr" : "?tr"}=w-450${calculateCropCoordinates(
    450,
    value?.coordinates?.mobile
  )}`;
