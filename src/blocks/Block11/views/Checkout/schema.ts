...
export const deliveryOptions = ["pickup", "dispatch"];

export const useSchema = () => {
  const { restaurant } = useSelector<TEleganteStore, TEleganteStore>((store) => store);
  const guest = restaurant?.cart?.guest; 

  const userInfoKeys: Array<"firstname" | "lastname" | "email" | "phone"> = ["firstname", "lastname", "email", "phone"];
  const addressKeys: Array<"addressline1" | "postcode" | "city" | "state" | "country"> = [
    "addressline1",
    "postcode",
    "city",
    "state",
  ...
  const initialState = () => {
    ...
    addressKeys.forEach((key) => (state[key] = (guest?.address?.[key] as string) ?? ""));
    state["payment method"] = "";
    state["delivery option"] = guest?.address?.uuid ? "dispatch" : "pickup";
