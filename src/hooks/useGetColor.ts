...
export const useGetColor = (configuration: CustomThemeProps, type: string) => {
  const { colors } = useSelector<TEleganteStore, TEleganteStore>(
    (store) => store
  );
...
export const useGetFonts = () => {
  const { fonts } = useSelector<TEleganteStore, TEleganteStore>(
    (store) => store
  );

  return (configuration: TBlockConfig["configuration"]) => {
